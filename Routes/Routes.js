...
router.get('/user/:username/:portfolioname', verify, (req, res) => {
    User.findOne({ username: req.params.username }).then((user) => {
        portfolio = user.portfolios.filter(
            (e) => e.alias == req.params.portfolioname
        )
...
router.get('/user/:username/:portfolioname/:walletname', verify, (req, res) => {
    try {
        User.findOne({ username: req.params.username }).then((user) => {
            ...
            wallet = portfolio[0].wallets.filter(
                (e) => e.alias == req.params.walletname
            )
...
router.post('/user/portfolios/wallets/:username', verify, (req, res) => {
    try {
        User.findOne({ username: req.params.username }).then((user) => {
            ...
            if (
                portfolio[0].wallets.every(
                    (wallet) =>
                        (wallet.alias != req.body.walletAlias) &
                        (wallet.address != req.body.walletAddress)
                )
            ) {
                portfolio[0].wallets.push({
                    alias: req.body.walletAlias,
                ...
                portfolio[0].wallets.every(
                    (wallet) => wallet.alias != req.body.walletAlias
                )
            )
                res.status(200).send({ error: 'Wallet alias already exist' })
            else if (
                portfolio[0].wallets.every(
                    (wallet) => wallet.address != req.body.walletAddress
                )
            )
                res.status(200).send({ error: 'Wallet address already exist' })
            else {
                res.status(200).send({ error: 'Wallet already exist' })
            }
        })
    } catch (e) {
        throw e
    ...
    User.findOne({ username: req.params.username }).then((user) => {
        ...
        user.save().then((data) =>
            res
                .status(200)
                .send({
                    message: `User ${req.params.username} is deleted`,
                    data: data,
                })
        )
    })
...
router.delete('/user/portfolios/wallets/:username', verify, (req, res) => {
    try {
        User.findOne({ username: req.params.username }).then((user) => {
            portfolio = user.portfolios.filter((e) => e.alias == req.body.alias)
            portfolio[0].wallets = portfolio[0].wallets.filter(
                (e) => e.alias != req.body.walletAlias
            )
...
router.post('/login', (req, response) => {
    User.findOne({ username: req.body.username }).then((dbres) => {
        if (!dbres) {
            ...
            bcrypt
                .compare(req.body.password, dbres.password)
                .then((isValid) => {
                    if (!isValid) {
                        response
                            .status(200)
...
router.post('/settings/portfolio/:username', verify, (req, res) => {
    try {
        User.findOne({ username: req.params.username }).then((user) => {
            ...
            const portfolio = user.portfolios.filter(
                (x) => x.alias == user.preferredPortfolio
            )
